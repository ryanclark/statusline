...
fn refresh_git_cache(cwd: &str) -> GitCache {
	...
	GitCache { branch, dirty, ahead, behind, stash_count }
}
...
fn fetch_git_ahead_behind(cwd: &str) -> Option<(u64, u64)> {
	let text = git_output(cwd, &["rev-list", "--left-right", "--count", "HEAD...@{upstream}"])?;
...
pub(super) fn git_ahead_behind(segment: &SegmentConfig, ctx: &RenderContext<'_>) -> Option<String> {
	...
	let up = if ctx.nerd_font { "\u{f062}" } else { "\u{2191}" };
	let down = if ctx.nerd_font { "\u{f063}" } else { "\u{2193}" };
...
pub(super) fn git_stash(segment: &SegmentConfig, ctx: &RenderContext<'_>) -> Option<String> {
	...
	let icon = format_icon(segment, Icon { unicode: "\u{2691}", nerd: "\u{f1b2}" }, YELLOW, ctx.nerd_font);
